...
> 每次都要手動複製整段錯誤訊息到瀏覽器或聊天工具，會讓排查流程中斷。這篇會在 Node-RED 建立一個報錯分析 API，輸入 `rescue` 後，由本機 AI 回傳原因說明與可供檢查的修復命令。
...
今天將本機 AI 接到終端機工作流，讓報錯內容可以透過 `rescue` 送進分析流程。

* **明天（Day 06）**：接著處理剪貼簿中的非結構化文字，將內容整理成 Markdown 表格。
