...
> 當本機 AI 可以操作檔案系統、終止程序或執行命令時，流程就需要明確的人工核准邊界。
> 例如 AI 判斷某個 2.4GB 檔案是暫存檔，但實際上它可能是剛產生的 Release ISO。若直接執行刪除，資料可能難以復原。
> 今天會加入 **Human-in-the-loop 審批門**：AI 先建立待確認工單，使用者透過 Windows Toast 開啟審批頁面，核准後才執行高風險動作。
...
| 操作風險等級                             | 典型場景                                                     | 系統策略                                        |
| :--------------------------------------- | :----------------------------------------------------------- | :---------------------------------------------- |
| 🟢**唯讀操作（Read-Only）**        | 讀取 Event Viewer 日誌、查詢記憶體、分析 Git 提交            | **可自動執行**（不需要人工核准）        |
...
> 動態生成的單次 Token 與 10 分鐘 TTL 可以降低工單被重複使用或長時間保留的風險。
...
在看到通知視窗後，打開瀏覽器輸入 `http://127.0.0.1:1880/api/hitl/dashboard`，即可查看待審工單並選擇批准或拒絕。
...
3. 點擊 **「✅ 批准執行」**：
   ...
   * 若重新整理同一個批准 URL，會顯示 **「❌ 無效或已核銷的工單 ID」**，避免同一工單被重複處理。
4. 點擊 **「❌ 拒絕」**：
   * 工單會從 Flow Context 中移除，顯示拒絕確認頁，AI 提議的動作不會被執行。
...
今天加入了防重放 Token、10 分鐘 TTL，以及以隔離取代直接刪除的處理方式，讓高風險動作在執行前需要人工核准。

* **明天（Day 20）**：接著把剪貼簿內容送進 AI 分析，處理程式碼、報錯與技術文件。