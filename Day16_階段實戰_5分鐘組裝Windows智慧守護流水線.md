...
## 技術演進：Day 08 雛形與 Day 16 整合版的差異

| 模組維度                 | ❌ Day 08 初階雛形（手動拼裝）                              | 🛡️ Day 16 全武裝 2.0（模組化集大成）                                                               |
| :----------------------- | :---------------------------------------------------------- | :--------------------------------------------------------------------------------------------------- |
| **AI 呼叫方式**    | 手動拉 3 顆節點（組裝指令、Exec、雙軌解包）                 | **直接調用 Day 14 的 `🤖 agy 推理核心大腦` Subflow**，減少重複節點。                    |
| **資料傳輸開銷**   | 原始 JSON 全部塞入 Prompt，冗餘雜訊多                       | **融合 Day 10 前置資料壓縮**：先保留分析需要的欄位，降低 Prompt 開銷。                           |
| **契約注入與轉義** | 需手動撰寫 Schema 字串，自行處理 CLI 雙重跳脫引號與雙軌解包 | **融合 Day 09 契約精神與 Day 14 抽象封裝**：標準傳入 `msg.schema`，底層自動跳脫與解包。      |
| **重複通知防護**   | 使用記憶體變數暫存，重開 Node-RED 容易遺失防抖狀態          | **融合 Day 13 本地 Context 狀態鎖**，用 10 分鐘冷卻視窗降低重複告警。                             |
...
> 平時只執行低成本的狀態檢查；未超過門檻時不呼叫 AI，超過門檻後啟動 10 分鐘冷卻防抖並收集進程快照。
...
[定時 3 分鐘 (Inject)] 
       ↓
【1. 資料蒐集 (Function)】 ➔ 使用 JS os 模組取得系統資訊
       ↓
...
> 常駐巡檢若每 3 分鐘啟動一次 PowerShell，會增加處理序啟動開銷。透過 Node.js 原生 `os` 模組，可以直接取得系統資訊，減少額外程序。
...
> **🔑 重點說明：前置處理的作用**
> 狀態鎖可以降低重複告警；欄位壓縮可以減少送入 Prompt 的資料量；再由 `--json-schema` 約束輸出型別，讓下游依固定欄位檢查結果。

---

### 步驟 6：調用共用 AI Subflow（`🤖 agy 推理核心大腦`）
...
今天將前 15 天的資料收集、分流、前置壓縮、狀態鎖、Schema 約束與 Subflow 串成一條完整流程，作為第一階段的整合範例。

* **明天（Day 17）**：接著處理 Windows Event Viewer 的應用程式崩潰事件，並依嚴重程度分流。
