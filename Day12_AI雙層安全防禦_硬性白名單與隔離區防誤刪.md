...
> 讓 AI 協助整理下載區或清理快取時，仍可能把重要檔案誤判成可處理項目。
> 因此刪除動作不能只依賴模型判斷。今天會加入硬性白名單與隔離區，讓高風險檔案先被阻擋或移出原位置儲存。
...
【第 2 道防線：Quarantine 隔離區】 ➔ 絕不物理刪除！安全搬移至隔離目錄，保留 30 天後悔藥
       ↓
【安全審計與存證追蹤】 ➔ 寫入 security_audit.log 磁碟日誌，保留操作紀錄供後續查閱
```
...
* **Node-RED 警告視窗**：
  ```text
  🛑 [安全攔截] 檔案「C:\Users\User\Downloads\.env」為受保護核心資產，AI 絕對不可操作！
  ```
* **節點狀態**：顯示紅色環狀圖示 `已物理阻斷: .env`，整條流程在第 1 道防線即刻終止，**下游隔離與刪除邏輯連碰都碰不到**！
* **審計日誌 (`C:\Users\User\SystemLogs\security_audit.log`)**：
  ```text
...
今天建立了硬性白名單與隔離區，讓檔案處理流程在執行前多一層檢查，也保留後續人工復原的空間。

* **明天（Day 13）**：接著評估哪些工作適合自動化，並加入避免重複執行的防呆設計。
