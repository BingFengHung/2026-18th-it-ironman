...
> 如果把 3,000 字的原始資料直接放進 Prompt，會增加輸入量，也可能讓關鍵欄位被雜訊淹沒。實際處理時間會受模型、硬體與資料內容影響。\
> 今天將用 **Node-RED 的 JSONata 表達式** 與 JavaScript 做前置資料整理，保留後續分析需要的欄位。
...
### 步驟 2：使用 Change 節點的 JSONata 表達式整理資料
...
3. **處裡方式單純**：JSONata 是專為 JSON 查詢與轉換設計的表達語言，執行效率極高且不涉及 VM 腳本編譯，減少記憶體 GC 壓力。
...
在後方的 Function 節點加入雙軌解包邏輯，處理 `structured_output` 或帶有 Markdown 包裹的字串：
...
最後接上一個 **`debug`** 節點，即可在 Node-RED 側邊欄查看結構化分析結果。

---

## 成果輸出範例：AI 分析結果

當我們將原本 3,420 字元的日誌整理為 142 字元送入後，本機 AI 回傳符合 Schema 的分析結果：
...
| **輸出 JSON 格式** | 可能包含額外字串 | 依 Schema 檢查欄位 | 下游依檢查結果處理 |
...
今天示範了在資料送入模型前先整理欄位，減少不必要的輸入內容。實際效能仍需依資料量與執行環境測試。
