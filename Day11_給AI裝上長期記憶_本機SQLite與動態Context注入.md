...
> 如果每次診斷只提供當下資料，模型就無法參考過去的事故與處理結果。
> 今天會將歷史紀錄存進本機 SQLite，再依目前任務查詢相關內容，注入後續 Prompt。
...
* **保留關聯式資料庫的時序與數值過濾，再把相關歷史資料提供給 AI 參考。**
...
【階段一：AI 自主規劃 SQL 查詢】 ➔ AI 根據異常與 Schema，自動推理出最適 SELECT 語句
      ↓
【中繼站：安全過濾與本機 SQLite 檢索】 ➔ 白名單正則攔截非 SELECT 指令，sqlite3.exe 取回 JSON
      ↓
...
### 中繼站：安全防護與本機 SQLite 檢索

讓 AI 動態產生 SQL 時，應限制只允許執行查詢，避免意外執行寫入或刪除操作。
...
* 搭配 `-json` 參數，SQLite 會將查詢結果輸出為 JSON 陣列，方便下游節點解析。
...
今天將歷史資料進行保存、查詢與 Context 注入串成一條流程，讓模型在分析目前事件時可以參考過往紀錄。

* **明天（Day 12）**：接著處理 AI 可能涉及檔案操作時的安全邊界，加入白名單與隔離區。
