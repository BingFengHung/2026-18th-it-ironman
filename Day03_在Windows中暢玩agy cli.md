...
* 認證成功後，Token 會安全儲存於本機憑證庫（位於 `~/.gemini` 目錄），之後可在終端機中直接呼叫 CLI。
...
| `--json-schema`                  | 傳入 JSON Schema 約束                  | 讓模型依指定欄位與型別回傳資料，方便下游解析                 |
...
2. **費用模式較單純**：每月固定訂閱費用，仍需依方案與使用限制評估成本。
3. **原生支援 `--json-schema`**：可以指定回傳欄位與型別，降低下游解析格式不一致的機會。
...
* **明天（Day 04）**：接著實作 Downloads 檔案分類與自動歸檔流程。
