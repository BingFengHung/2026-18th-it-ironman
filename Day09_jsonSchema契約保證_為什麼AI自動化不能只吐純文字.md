...
> 在前面幾天的實戰中，你一定注意到了在呼叫 `agy CLI` 的時候，都會帶上一個特殊的參數：`--json-schema`。
> 初學者常問一個直覺的問題：「直接叫 AI 輸出純文字或 Markdown 不就好了嗎？為什麼一定要大費周章定義 JSON Schema？」
> 今天將說明 JSON Schema 在自動化管線中的作用，以及為什麼下游節點不能只依賴自由格式文字。
...
`--json-schema` 是約束模型輸出格式的機制，可以讓下游節點依照固定欄位處理結果。

* **明天（Day 10）**：接著處理長日誌的前置資料整理，介紹 JSONata 與 JavaScript 的使用方式。
