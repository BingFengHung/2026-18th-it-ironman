# Day 08：AI 實戰 5：電腦突然風扇狂轉？AI 輔助根因診斷（RCA 分析）
...
>
> 今天將實作一條資源異常分析流程：平時定期檢查系統狀態，偵測到負載升高時再收集相關資訊，交給本機 AI 整理可能原因與處理建議。
...
| 比較項目             | 傳統手動開工作管理員                  | 🤖 AI 輔助 RCA 分析                                           |
| :------------------- | :------------------------------------ | :-------------------------------------------------------------------- |
| **反應方式**   | 卡頓時手動開啟工作管理員並查找進程 | **背景定期檢查**，超過門檻後觸發分析                       |
| **資訊可讀性** | 滿滿英文進程名與十六進位代碼，看不懂  | **白話文解釋成因**（如說明是 Docker 快取未釋放）                |
| **操作指引**   | 不敢隨便關閉進程，怕系統藍屏          | **指名道姓可疑進程**，給出安全處置指令（如 `wsl --shutdown`） |
| **資源開銷**   | 輪詢頻率過高時會增加負載              | **只在超過門檻時呼叫 AI，平時不送出模型請求**                       |
...
今天完成了資源異常觸發、資料收集與 RCA 分析的串接。實際負載與處理時間仍會依排程頻率、收集資料量及本機硬體而變化。

* **明天（Day 09）**：將接著說明 `--json-schema` 如何約束 AI 回傳資料，讓下游節點能穩定解析。
